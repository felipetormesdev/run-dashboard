...
async function fetchAthlete(token: string) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Erro ao buscar atleta: ${res.status} - ${text}`);
  }
  ...
  try {
    ...
    try {
      ...
      if (err.message === "Token expirado") {
        ...
        return NextResponse.json({ message: "Token renovado automaticamente", athlete });
      }
      throw err;
    }
  } catch (err: any) {
